    ...
    , _processing(false)
{
    ...
    _gpp.setProgram(configObject["cpp"].toObject()["compiler_path"].toString(
        "/usr/bin/g++"));
    connect(&_gpp,
            static_cast<void (QProcess::*)(int, QProcess::ExitStatus)>(
                &QProcess::finished),
            this,
            &CppCommand::gppFinished);
    connect(&_app,
            static_cast<void (QProcess::*)(int, QProcess::ExitStatus)>(
                &QProcess::finished),
            this,
            &CppCommand::appFinished);
}
...
void CppCommand::dispatch(QDiscordMessage message, QStringList args)
{
    ...
    if(!gppExecutable.exists())
    {
        ...
        outputText +=
            " does not seem to be a valid file. Set the "
            "cpp.compiler_path config file variable to a valid compiler "
            "path.";
        message.edit(outputText);
        return;
    }
    ...
    if(_processing)
    {
        message.edit("Already processing.");
        return;
    }
    ...
    {
        message.edit("Code formatting error.");
        return;
    }
    int i = QString("```cpp\n").length();
    ...
    {
        message.edit("Could not open temporary file.");
        return;
    }
    file.write((code + "\n").toUtf8());
    file.flush();
    file.close();
    // Add some of our own arguments to the compiler so it knows where our
    // source  is and what our executable should be named.
    ...
    QDiscordMessage::create(
        _discord.rest(), message.channelId(), "Compiling...");
    _gpp.start();
...
void CppCommand::gppFinished(int exitCode, QProcess::ExitStatus exitStatus)
{
    ...
    {
        QDiscordMessage::create(_discord.rest(),
                                _currentMessage.channelId(),
                                "Compile failed:\n```\n"
                                    + _gpp.readAllStandardOutput()
                                    + _gpp.readAllStandardError() + "```");
        _processing = false;
        return;
    }
    _gpp.readAll();
    ...
    QDiscordMessage::create(
        _discord.rest(), _currentMessage.channelId(), "Running...");
    _app.start();
    QTimer::singleShot(3000, [this]() {
...
void CppCommand::appFinished(int exitCode, QProcess::ExitStatus exitStatus)
{
    ...
    {
        QDiscordMessage::create(
            _discord.rest(), _currentMessage.channelId(), "Output too large.");
        _processing = false;
        return;
    }
    QDiscordMessage::create(
        _discord.rest(), _currentMessage.channelId(), output);
    _processing = false;
}
