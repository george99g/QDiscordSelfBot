...
void UrbanCommand::dispatch(QDiscordMessage message, QStringList args)
{
    if(args.length() != 1)
    {
        message.edit("One argument is required.");
        return;
    }
    // Construct the URL
    ...
    connect(reply, &QNetworkReply::finished, [=]() mutable {
        ...
        {
            message.edit("Error: " + reply->errorString());
            reply->deleteLater();
            return;
        }
        ...
        {
            message.edit("Could not find a definition.");
            reply->deleteLater();
            return;
        }
        ...
        if(definition.length() > 2000)
            definition =
                "Definition too long. Visit " + object["permalink"].toString();
